...
class ImageDataGenerator(object):
    ...
    def flow_from_dataset(self, dataset, img_dir,
                          target_size=(256, 256), color_mode='rgb',
                          classes=None, class_mode='categorical',
                          batch_size=32, shuffle=True, seed=None,
                          save_to_dir=None,
...
class DatasetIterator(Iterator):
    ...
    def __init__(self, dataset, img_dir, image_data_generator,
                 target_size=(256, 256), color_mode='rgb',
                 dim_ordering='default',
                 classes=None, class_mode='categorical',
                 batch_size=32, shuffle=True, seed=None,
        ...
        if not classes:
            classes = list(set(dataset["category_id"]))

        self.nb_class = len(classes)
        self.class_indices = dict(zip(classes, range(len(classes))))
        self.nb_sample = len(dataset)
        print('%d images belonging to %d classes.' % (self.nb_sample, self.nb_class))
